...
def complex_max_pool2d(
    ...
    return (
        max_pool2d(
            input_r, kernel_size, stride, padding, dilation, ceil_mode, return_indices
        ),
        max_pool2d(
            input_i, kernel_size, stride, padding, dilation, ceil_mode, return_indices
        ),
    )
...
def complex_dropout(input_r, input_i, p=0.5, training=True, inplace=False):
    return (
        dropout(input_r, p, training, inplace),
...
def complex_dropout2d(input_r, input_i, p=0.5, training=True, inplace=False):
    return (
        dropout2d(input_r, p, training, inplace),
